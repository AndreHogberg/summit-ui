...
public sealed class AccordionContext
{
    ...
    /// </summary>
    private string AccordionId { get; } = $"summit-accordion-{Guid.NewGuid():N}";
    ...
    /// </summary>
    /// <param name="value">The value identifying the accordion item.</param>
    public void UnregisterTrigger(string value) => _triggers.RemoveAll(t => t.Value == value);
    ...
    {
        TriggerRegistration? trigger = _triggers.FirstOrDefault(t => t.Value == value);
    ...
    /// </summary>
    private IReadOnlyList<string> GetEnabledTriggerValues() =>
        _triggers.Where(t => !t.IsDisabled).Select(t => t.Value).ToList();
    ...
    {
        IReadOnlyList<string> enabledTriggers = GetEnabledTriggerValues();
