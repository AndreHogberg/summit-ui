...
/// </summary>
public class SmScrollAreaScrollbar : ComponentBase, IAsyncDisposable
{
    [Inject]
    ...
    protected override void BuildRenderTree(RenderTreeBuilder builder)
    {
        ...
        // Core attributes
        builder.AddAttribute(seq++, "id", Context.GetScrollbarId(Orientation));
        builder.AddAttribute(seq++, "data-summit-scroll-area-scrollbar", true);
        builder.AddAttribute(seq++, "data-orientation", Orientation.ToString().ToLowerInvariant());
        builder.AddAttribute(seq++, "data-state", _currentState);

        // Orientation-specific data attribute for easier CSS targeting
        if (Orientation == ScrollAreaOrientation.Vertical)
        {
            builder.AddAttribute(seq++, "data-summit-scroll-area-scrollbar-y", true);
        }
        else
        {
            builder.AddAttribute(seq++, "data-summit-scroll-area-scrollbar-x", true);
        }

        // Element reference
        builder.AddElementReferenceCapture(seq++, capturedRef => _elementRef = capturedRef);

        // Cascading value to pass orientation and element ref to thumb
        builder.OpenComponent<CascadingValue<SmScrollAreaScrollbar>>(seq++);
        builder.AddComponentParameter(seq++, "Value", this);
